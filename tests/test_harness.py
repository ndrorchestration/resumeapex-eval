...
def test_fixture_loads_and_scores():
    ...
    result = evaluate(cases, fixture_adapter, runs=2, analysis_seed=7, bootstrap_resamples=200)
...
def test_fixture_is_deterministic():
    cases = load_cases(DATA)
    a = evaluate(cases, fixture_adapter, runs=2, analysis_seed=99, bootstrap_resamples=200)
    b = evaluate(cases, fixture_adapter, runs=2, analysis_seed=99, bootstrap_resamples=200)
    assert a["confidence_intervals"] == b["confidence_intervals"]
    assert a["results"] == b["results"]


def test_negative_control_does_not_match_expected_metrics():
    cases = load_cases(DATA)
    with pytest.raises(ValueError):
        evaluate(cases, strict_zero_adapter, runs=1, analysis_seed=1, bootstrap_resamples=50)
